...
const main = async () => {
  ...
  // Deployment of the Deposit contract
  ...
  // We subscribe to the Deposit contract events
  depositContract.events.creditEvent().on("data", (event) => {
  ...
  depositContract.events.offerEvent().on("data", (event) => {
  ...
  depositContract.events.orderEvent().on("data", (event) => {
    let buyer = event.returnValues.buyer;
    let title = event.returnValues.title;
    console.log(`Buyer ${buyer}  | Order    | ${title}`);
  });

  depositContract.events.completeEvent().on("data", (event) => {
    let buyer = event.returnValues.buyer;
    let title = event.returnValues.title;
    console.log(`Buyer ${buyer}  | Complete | ${title}`);
  });

  depositContract.events.complainEvent().on("data", (event) => {
    let buyer = event.returnValues.buyer;
    let title = event.returnValues.title;
  ...
  orderIdTShirt = await depositContract.methods.getLastOrderId().call();
  ...
  await depositContract.methods
    ...
    orderIdHoody = await depositContract.methods.getLastOrderId().call();
  ...
  await depositContract.methods
    .order('Coffee')
    .send({ from: buyers[0], gas: '6000000' });
  orderIdCoffee = await depositContract.methods.getLastOrderId().call();
  ...
  let balanceSeller1 = await depositContract.methods.getAccountBalance(sellers[1]).call();
