...
const compiledDeposit = require('../build/Deposit.json');
...
beforeEach(async () => {
  ...
  depositContract = await new web3.eth.Contract(compiledDeposit.abi)
    .deploy({ data: compiledDeposit.evm.bytecode.object, arguments: [] })
    .send({ from: accounts[0], gas: '6000000' });
});

describe('Deposit contract test', () => {
  it('deploys an Deposit contract', () => {
    // We check that the Deposit contract has been deployed
    console.log(`The Deposit smart contract has the ${depositContract.options.address} address`)
    assert.ok(depositContract.options.address);
  });

  it('a buyer makes a credit', async () => {
    // Buyer 0 makes a credit of 20 Ethers
    // We read the balance before the credit
    let balanceBuyer0 = await depositContract.methods.getAccountBalance(buyers[0]).call();
    ...
    balanceBuyer0 = await depositContract.methods.getAccountBalance(buyers[0]).call();
  ...
  it('a seller offers an item', async () => {
    ...
    let checkItemPrice = await depositContract.methods.getItemPrice('Coffee').call();
  ...
  it('a buyer orders an item', async () => {
    ...
    let lastOrderId = await depositContract.methods.getLastOrderId().call();
    let balanceLastOrder = await depositContract.methods.getOrderBalance(lastOrderId).call();
    ...
    let totalHeld = await depositContract.methods.getTotalHeld().call();
    let totalHeldEthers = Web3.utils.fromWei(totalHeld, 'ether');
    console.log(`The total amount held in deposit is ${totalHeldEthers} ethers`);
  ...
  it('a buyer completes an order', async () => {
    ...
    let balanceSeller0 = await depositContract.methods.getAccountBalance(sellers[0]).call();
    ...
    // We read the last order id
    balanceLastOrder = await depositContract.methods.getOrderBalance(lastOrderId).call();
    ...
    balanceSeller0 = await depositContract.methods.getAccountBalance(sellers[0]).call();
